...
public class CustomerController : Controller
{
    ...
    [ProducesResponseType(typeof(Common.Customer), 200)]
    [ProducesResponseType(typeof(string), 400)]
    async public Task<IActionResult> CustomerGetById(string customerId)
    {
        ...
        var response = new Common.Customer()
        {
            ...
            Gender = customer.Gender.HasValue ? customer.Gender.Value : null,
            Active = customer.Active.HasValue ? customer.Active.Value : null,
    ...
    [ProducesResponseType(typeof(Common.SearchResponse<Common.Customer>), 200)]
    [ProducesResponseType(typeof(string), 400)]
    async public Task<IActionResult> CustomerSearch([FromBody] Common.Search model)
    {
        ...
        {
            query = query.Where(i => (i.Name != null && i.Name.ToLower().Contains(model.FilterText.ToLower())) ||
                                    (i.Email != null && i.Email.ToLower().Contains(model.FilterText.ToLower())) ||
                                    (i.Phone != null && i.Phone.ToLower().Contains(model.FilterText.ToLower())) ||
                                    (i.Address != null && i.Address.ToLower().Contains(model.FilterText.ToLower())) ||
                                    (i.State != null && i.State.ToLower().Contains(model.FilterText.ToLower())) ||
                                    (i.Postal != null && i.Postal.ToLower().Contains(model.FilterText.ToLower())) ||
                                    (i.Notes != null && i.Notes.ToLower().Contains(model.FilterText.ToLower())));
        }
        ...
        response.Results = dataResponse.Select(c => new Common.Customer()
        {
            ...
            State = c.State,
            Postal = c.Postal,
            Gender = c.Gender ?? Common.Gender.NotSpecified,
